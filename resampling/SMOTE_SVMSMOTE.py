...
sm = eval('SVMSMOTE')(random_state=42)
X_res, y_res = sm.fit_resample(X, y)
print('SVMSMOTE', 'resampled class distribution is ', sorted(Counter(y_res).items()))
...
result.to_csv('datasets/' + dataname + '_' + 'SVMSMOTE' + '.csv', index=False, header=False) # First column, Index, is removed 

# This output is for information about the number of resamples
with open('datasets/' + dataname + '_' + 'SVMSMOTE' + '_info.txt', 'w') as filehandle:
