...
sm = eval('SMOTE')(random_state=42)
X_res, y_res = sm.fit_resample(X, y)
print('SMOTE', 'resampled class distribution is ', sorted(Counter(y_res).items()))
...
result.to_csv('datasets/' + dataname + '_' + 'SMOTE' + '.csv', index=False, header=False) # First column, Index, is removed 
#result.to_csv(dataname + '_' + "SMOTE.csv")

# This output is for information about the number of resamples
with open('datasets/' + dataname + '_' + 'SMOTE' + '_info.txt', 'w') as filehandle:
